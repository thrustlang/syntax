...
```thrust
intrinsic("llvm.assume") assume(condition: bool);
intrinsic("llvm.sqrt.f32") sqrt(value: f32) f32;
```

The first string is the LLVM intrinsic name, the identifier after it is the name used in Thrust code, and the rest is the parameter list and return type.
